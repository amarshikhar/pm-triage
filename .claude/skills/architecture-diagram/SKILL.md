...
## Local additions (this repository)

Everything above is upstream (`github.com/konraddzbik/architecture-diagram-skill`,
MIT — see `LICENSE`). Vendored here so it is available in any session on this
repo. Two references and three scripts were added locally:

| File | Read it when |
|---|---|
| `references/static-svg-diagrams.md` | The diagram must render inline on GitHub (README hero, docs figure). Covers the generate-don't-hand-write method, text-width estimation, the headless-Chromium verification loop, and GitHub's SVG constraints — the static case the skill above deliberately excludes. |
| `references/applying-the-template.md` | Before filling `assets/template.html`. Mode renaming, the two `CONFIG` lists that silently keep stale flow keys, how many columns actually fit beside the side panel, and how to screenshot a specific flow/mode. |
| `scripts/gen_hero.py`, `scripts/gen_detail.py` | Regenerate the committed static SVGs in `docs/diagrams/`. |
| `scripts/build_interactive.py` | Regenerate `docs/diagrams/architecture-interactive.html` from `assets/template.html`. |

All three scripts write into `docs/diagrams/` relative to the repo root and are
idempotent: after running them `git status` must stay clean. If it does not, the
committed diagram and its script have drifted — fix the script, not the output.
