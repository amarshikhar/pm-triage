...
**What it does:** IoT telemetry streams from a fleet of 8 simulated machines
plus one real-data pump (`PMP-03`, above) →
...
```bash
# backend (Python 3.11)
cd 01-revenue/pm-triage/backend
...
It drives the real pipeline (real simulator/replayer, real detector, real agent
tool loop) on a machine with a known fault, then scores the case that comes out.
Committed runs: `backend/eval-report.json` + `backend/eval-report-real.json`,
regenerated by the **`Eval` GitHub workflow** (which runs live in CI, where the
key reaches OpenRouter). Detection is 100% everywhere; root-cause accuracy:

| | mock (scripted) | live (Sonnet 4.5) |
|---|---|---|
| synthetic top-1 | 58.3% | **70.8%** (+12.5pp), ECE **0.043** |
| real SKAB top-1 | **50.0%** | 31.2% (−18.8pp), ECE 0.398 |

**The finding:** the LLM helps and is well-calibrated on synthetic faults, but on
**real SKAB data it underperforms the scripted baseline and is badly overconfident**
— the model's accuracy *and* its confidence do not transfer to real data it
wasn't built around. That is the hardest evidence for why the human gate is
structural, and why you measure on real data before trusting a model. (Real n=16,
so the delta is directional; the calibration blow-up is the robust signal.) The
in-app **Evaluation page** renders this — confusion matrix included.
